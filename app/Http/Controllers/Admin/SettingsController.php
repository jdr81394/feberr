...
class SettingsController extends Controller
{
	...
	public function general_settings()
    {
		...
		$sid = 1;
		$setting['setting'] = Settings::editGeneral($sid);
		$currencyData['currency'] = Settings::getcurrencyData();
		return view('admin.general-settings', [ 'setting' => $setting, 'sid' => $sid, 'currencyData' => $currencyData]);
	 ...
	 public function update_general_settings(Request $request)
	{
		 ...
		 $site_primary_color = $request->input('site_primary_color');
		 $site_secondary_color = $request->input('site_secondary_color');
		 ...
		 $site_blog_adbanner_link = $request->input('site_blog_adbanner_link');
		 ...
		 $site_category_newest_files = $request->input('site_category_newest_files');
		 if(!empty($request->input('site_layout')))
		 {
		    $site_layout = $request->input('site_layout');
		 }
		 else
		 {
		   $site_layout = "";
		 ...
		 $request->validate([
							...
							'site_blog_adbanner' => 'mimes:jpeg,jpg,png|max:'.$image_size,
							'site_count_bg' => 'mimes:jpeg,jpg,png|max:'.$image_size,
		...
		if ($request->hasFile('site_favicon')) {
		  ...
		  if ($request->hasFile('site_blog_adbanner')) {
		     
			Settings::dropadBanner($sid); 
		   
			$image = $request->file('site_blog_adbanner');
			$img_name = time() . '11.'.$image->getClientOriginalExtension();
			$destinationPath = public_path('/storage/settings');
			$imagePath = $destinationPath. "/".  $img_name;
			$image->move($destinationPath, $img_name);
			$blog_banner_image = $img_name;
		  }
		  else
		  {
		     $blog_banner_image = $request->input('save_blog_adbanner');
		  }
		  
		  
		  if ($request->hasFile('site_count_bg')) {
		     
			Settings::dropcountBanner($sid); 
		   
			$image = $request->file('site_count_bg');
			$img_name = time() . '009.'.$image->getClientOriginalExtension();
			$destinationPath = public_path('/storage/settings');
			$imagePath = $destinationPath. "/".  $img_name;
			$image->move($destinationPath, $img_name);
			$site_count_bg = $img_name;
		  }
		  else
		  {
		     $site_count_bg = $request->input('save_count_bg');
		  }
		...
		$data = array('site_title' => $site_title, 'site_desc' => $site_desc, 'site_keywords' => $site_keywords, 'site_currency' => $site_currency, 'site_favicon' => $fav_image, 'site_logo' => $logo_image, 'site_primary_color' => $site_primary_color, 'site_secondary_color' => $site_secondary_color, 'site_button_color' => $site_button_color, 'site_menu_category' => $site_menu_category, 'site_banner' => $banner_image, 'site_banner_heading' => $site_banner_heading, 'site_banner_subheading' => $site_banner_subheading, 'item_approval' => $item_approval, 'site_item_per_page' => $site_item_per_page, 'site_blog_adbanner' => $blog_banner_image,'site_blog_adbanner_link' => $site_blog_adbanner_link, 'site_post_per_page' => $site_post_per_page, 'site_range_min_price' => $site_range_min_price, 'site_range_max_price' => $site_range_max_price, 'office_address' => $office_address, 'office_email' => $office_email, 'office_phone' => $office_phone, 'site_count_bg' => $site_count_bg, 'site_newest_files' => $site_newest_files, 'site_category_newest_files' => $site_category_newest_files, 'site_layout' => $site_layout, 'site_flash_end_date' => $site_flash_end_date, 'site_free_end_date' => $site_free_end_date, 'site_newsletter' => $site_newsletter, 'site_watermark' => $site_watermark, 'google_analytics' => $google_analytics, 'watermark_option' => $watermark_option, 'multi_language' => $multi_language, 'email_verification' => $email_verification, 'payment_verification' => $payment_verification, 'home_blog_display' => $home_blog_display, 'site_blog_display' => $site_blog_display);
	...
	public function currency_settings()
    {
		...
		$currency['data'] = Settings::getcurrencyData();
		return view('admin.currency-settings',[ 'currency' => $currency]);
    }
	...
	public function add_currency()
	{
	 ...
	 public function edit_currency($cid)
	{
	   
	   $edit['currency'] = Settings::editCurrency($cid);
	   return view('admin.edit-currency', [ 'edit' => $edit, 'cid' => $cid]);
	}
	...
	public function save_currency(Request $request)
	{
		 ...
		 $rules = array(
				...
				'currency_code' => ['required', 'max:3', Rule::unique('currency') -> where(function($sql){ $sql->where('currency_code','!=','');})],
	     );
		...
		$data = array('currency_code' => $currency_code);
            ...
            Settings::savecurrencyData($data);
            return redirect()->back()->with('success', 'Insert successfully.');
            
 
       } 
     
    return redirect('/admin/add-currency');
  }
  
  
  public function delete_currency($cid){

      
	  
      Settings::deleteCurrencydata($cid);
  ...
  public function update_currency(Request $request)
	{
	   ...
	   $currency_code = $request->input('currency_code');
		 ...
		 $rules = array(
				'currency_code' => ['required', 'max:3', Rule::unique('currency') ->ignore($cid, 'currency_id') -> where(function($sql){ $sql->where('currency_code','!=','');})],
		...
		$data = array('currency_code' => $currency_code);
			...
			Settings::updatecurrencyData($cid, $data);
            return redirect()->back()->with('success', 'Update successfully.');
            
 
       } 
     
       return redirect('/admin/currency-settings');
  ...
  public function email_settings()
   ...
   public function highlights()
	...
	public function update_highlights(Request $request)
	{
		...
		$data = array('site_icon1' => $site_icon1, 'site_icon2' => $site_icon2, 'site_icon3' => $site_icon3, 'site_icon4' => $site_icon4, 'site_text1' => $site_text1, 'site_text2' => $site_text2, 'site_text3' => $site_text3, 'site_text4' => $site_text4);
